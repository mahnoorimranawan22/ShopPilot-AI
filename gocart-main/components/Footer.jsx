    ...
    const linkSections = [
        ...
        },
        {
            ...
            links: [
                ...
                { text: "Become a Seller", path: '/pricing', icon: null },
        ...
        },
        {
            ...
            links: [
                ...
                { text: "contact@shoppilot.ai", path: '/', icon: PhoneIcon },
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-200 text-slate-500">
                    <div>
                        ...
                        <p className="max-w-[410px] mt-6 text-sm">Welcome to ShopPilot AI, your intelligent shopping destination for the latest and smartest gadgets. Powered by AI-driven recommendations, we bring you the best in innovation — all in one place.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <motion.div
                                    ...
                                    whileTap={{ scale: 0.9 }}
                                >
                                    <Link href={item.link} className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:bg-orange-50 transition-colors rounded-full text-slate-500 hover:text-orange-500">
                                        <item.icon />
                                    </Link>
                                </motion.div>
                            ))}
                                ...
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="hover:underline transition hover:text-orange-500">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-4 text-sm text-slate-500 text-center">
                    Copyright 2025 &copy; ShopPilot AI All Rights Reserved.
                </p>
            </div>
        </footer>
    );
