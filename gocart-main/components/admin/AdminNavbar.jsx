    ...
    return (
        <div className="flex items-center justify-between px-12 py-3 border-b border-slate-200 transition-all">
            <Link href="/" className="relative text-3xl font-bold tracking-tight text-slate-800 flex items-center gap-0">
                ...
                <p className="absolute text-xs font-semibold -top-1 -right-13 px-3 p-0.5 rounded-full flex items-center gap-2 text-white bg-indigo-500">
                    Admin
                </p>
            </Link>
            <div className="flex items-center gap-4">
                <NotificationBadge />
                <p className="text-sm text-slate-600">Hi, {userData?.name?.split(' ')[0] || 'Admin'}</p>
            </div>
        </div>
    )
}
